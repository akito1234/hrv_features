...
def resp(samples, resolution=10):
	"""Converts raw RESP values into original physical unit (µV).

	BITalino Sensor: Respiration (RESP) Sensor Data Sheet 

	See sensor datasheet for more information:
	https://bitalino.com/datasheets/PZT_Sensor_Datasheet.pdf

	[TRANSFER FUNCTION]
	PZT(%) = (((ADC/2^n)- 1/2) *(10^2))
	...
	# Compute & check output
	resp_samples = np.asarray([((float(s) / 2 ** resolution) - 0.5) * (10 ** 2) for s in samples])
	_check_ranges(resp_samples, 'RESP')
	return resp_samples
